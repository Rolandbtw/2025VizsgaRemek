...
public class Bow : MonoBehaviour
{
    ...
    private bool isWindedUp = false;
    private bool isWindedUpUlt=false;
    private float timer;

    private Inventory inventoryScript;
    private Runes runes;
    [SerializeField] CooldownSignal cooldownSignal;
    Sounds soundScript;
    ...
    private void Update()
    {
        if (movementActions.PlayerMap.Attack.triggered && !runes.inventoryIsOpened && Time.timeScale != 0 && !inventoryScript.usingWeapon)
        {
            windUp = StartCoroutine(WindUpBow());
        }
        if (movementActions.PlayerMap.Attack.phase == InputActionPhase.Waiting && !runes.inventoryIsOpened && Time.timeScale != 0)
        {
            if (isWindedUp)
            {
        ...
        if (movementActions.PlayerMap.UltAttack.triggered && timer < Time.time && !runes.inventoryIsOpened && !inventoryScript.usingWeapon && Time.timeScale != 0)
        {
            windUpUlt = StartCoroutine(WindUpBowUlt());
        }
        if (movementActions.PlayerMap.UltAttack.phase == InputActionPhase.Waiting && !runes.inventoryIsOpened && Time.timeScale != 0)
        {
            if (isWindedUpUlt)
            {
                soundScript.MakeSound("bowSound", 0.5f);
