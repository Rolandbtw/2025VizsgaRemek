...
public class Spear : MonoBehaviour
{
    ...
    private void Update()
    {
        if (movementActions.PlayerMap.UltAttack.triggered && !isShotOut && timer < Time.time && !runes.inventoryIsOpened && Time.timeScale != 0 && !isTouchingWall && !inventoryScript.usingWeapon)
        {
        ...
        if (movementActions.PlayerMap.UltAttack.triggered && isTouchingWall && isShotOut && !runes.inventoryIsOpened && Time.timeScale != 0)
        {
            soundScript.MakeSound("spearSound", 0.5f);
            canComeBack = true;
        }
