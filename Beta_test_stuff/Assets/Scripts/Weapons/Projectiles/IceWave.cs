...
public class IceWave : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Enemy") && !collision.GetComponent<EnemyHealth>().isFroozen)
        {
            ...
            IceCube iceCubeScript = iceCubeClone.GetComponent<IceCube>();
            iceCubeScript.isAimedAtPlayer = false;
            iceCubeScript.fullSize = new Vector3(4, 4, 1);
            iceCubeScript.waitDuration = 10;
